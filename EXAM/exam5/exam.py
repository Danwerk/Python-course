...
def reverse_subword(s: str, subword: str) -> str:
    ...
    if subword not in s:
        ...
        k = s.find(subword, 0)
        reversed_substring = subword[::-1]
        final = s[0:k] + reversed_substring + s[k + len(subword):]
        return final
...
def sum_of_multipliers(first_num: int, second_num: int, limit: int) -> int:
    ...
    for i in range(first_num, limit + 1, first_num):
        first_num_list.append(i)
    for j in range(second_num, limit + 1, second_num):
        second_num_list.append(j)

    # check for duplicates
    for num in first_num_list:
        if num in second_num_list:
            second_num_list.remove(num)

    return sum(first_num_list) + sum(second_num_list)


# print(sum_of_multipliers(3, 5, 20))  # 98
...
class Store:
    ...
    def can_add_book(self, book: Book) -> bool:
        ...
        if book.rating >= self.rating and can_add == True:
            return True
        else:
            return False
    ...
    def get_most_popular_book(self) -> list:
        ...
        """
        ret = []
        sorted_books = sorted(self.books, key=lambda x: x.rating, reverse=True)
        most_popular_rating = sorted_books[0].rating
        for i in sorted_books:
            if i.rating == most_popular_rating:
                ret.append(i)
        return ret
